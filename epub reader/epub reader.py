...
FFMPEG_DIR = os.path.join(os.getenv('APPDATA') or os.path.expanduser('~'), 'epubReader', 'ffmpeg')  # FFmpeg安装目录
FFMPEG_PATH = os.path.join(FFMPEG_DIR, 'bin', 'ffmpeg.exe') if sys.platform == 'win32' else 'ffmpeg'  # FFmpeg可执行文件路径
# FFmpeg下载地址（多个镜像源，按顺序尝试）
FFMPEG_DOWNLOAD_URLS = [
    "https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip",
    "https://ghproxy.com/https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip",  # GitHub加速镜像
    "https://mirror.ghproxy.com/https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip",  # GitHub镜像2
    "https://gh.api.99988866.xyz/https://github.com/BtbN/FFmpeg-Builds/releases/download/latest/ffmpeg-master-latest-win64-gpl.zip",  # GitHub镜像3
...
def merge_mp3_to_m4b(mp3_files, output_m4b, metadata, cover_data=None, ffmpeg_path=None, chapter_titles=None):
    ...
    try:
        ...
        try:
            ...
            print("错误: 未找到FFmpeg。请安装FFmpeg并确保在系统PATH中。", file=sys.stderr)
            return False
        
        if not mp3_files:
            print("错误: 没有MP3文件可合并", file=sys.stderr)
            return False
        ...
        for mp3_file in mp3_files:
            try:
                ...
                if duration_match:
                    ...
                    chapter_durations.append(total_seconds)
                else:
                    print(f"警告: 无法获取 {os.path.basename(mp3_file)} 的时长，使用默认值")
                    chapter_durations.append(0)
            except Exception as e:
                print(f"警告: 获取 {os.path.basename(mp3_file)} 时长失败: {e}")
                chapter_durations.append(0)
        ...
        if result.returncode != 0:
            print(f"FFmpeg合并失败: {result.stderr.decode('utf-8', errors='ignore')}", file=sys.stderr)
            return False
        ...
        if result.returncode != 0:
            print(f"FFmpeg添加元数据失败: {result.stderr.decode('utf-8', errors='ignore')}", file=sys.stderr)
            return False
...
class EpubToMp3App:
    ...
    def create_mp3_converter_ui(self):
        ...
        ffmpeg_status_frame.grid(row=1, column=0, columnspan=3, sticky=tk.W, padx=5, pady=(5, 5))
    ...
    def _generate_ebook_thread(self, params):
        ...
        try:
            for i, chapter in enumerate(chapters, 1):
                ...
                if result:
                    if update_mp3_tag(output_mp3, output_lrc, cover_data, title, artist, album):
                        ...
                        mp3_files.append(output_mp3)  # 收集MP3文件用于M4B
                    else:
                        self.message_queue.put(("log", f"生成成功但标签失败: {title}"))
                        mp3_files.append(output_mp3)  # 即使标签失败也收集
                else:
            ...
            if output_format == "M4B" and mp3_files:
                ...
                if merge_mp3_to_m4b(mp3_files, m4b_output, metadata, cover_data, ffmpeg_path, 
